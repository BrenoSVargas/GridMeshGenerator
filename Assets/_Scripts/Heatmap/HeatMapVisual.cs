...
public class HeatMapVisual : MonoBehaviour
{
    ...
    public void SetGrid(GridLogic grid)
    {
        this.grid = grid;
        UpdateHeatMapVisual();
    ...
    private void Grid_OnGridValueChanged(object sender, GridLogic.OnGridValueChangedEventArgs e)
    {
        updateMesh = true;
    }
    ...
    public void UpdateHeatMapVisual()
    {
        ...
        for (int x = 0; x < grid.GetWidth(); x++)
        {
            for (int y = 0; y < grid.GetHeight(); y++)
            {
                ...
                float gridValueNormalized = (float)gridValue / GridLogic.HEAT_MAP_MAX_VALUE;
                Vector2 gridValueUV = new Vector2(gridValueNormalized, 0f);
                AddToMeshArrays(vertices, uvs, triangles, index, grid.GetWorldPosition(x, y) + quadSize * 0.5f, 0f, quadSize, gridValueUV, gridValueUV);
            }
        }
    ...
    public void CreateEmptyMeshArrays(int quadCount, out Vector3[] vertices, out Vector2[] uvs, out int[] triangles)
    {
